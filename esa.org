...
#+begin_src latex
  ...
  the 64-bit version of the divsufsort library.
#+end_src
#+begin_src go <<Cgo>>=
...
#+end_src
#+begin_src go <<Functions>>=
  func Lcp(t []byte, sa []int64) []int64 {
	  n := int64(len(t))
	  lcp := make([]int64, n)
...
#+end_src
#+begin_src latex
  Similarly, getting the $\mbox{lcp}$ array is just one function call.
#+end_src
